...
class HugoserverCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        try:
            out = subprocess.Popen("hugo server --theme="+theme+"--buildDrafts --watch --port="+server[
                                   "PORT"], stderr=subprocess.STDOUT, universal_newlines=True)
...
class HugogetthemesCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        setvars()
        try:
            out = subprocess.Popen("git clone --recursive https://github.com/spf13/hugoThemes.git "+os.path.join(
                path, sitename, "themes"), stderr=subprocess.STDOUT, universal_newlines=True)
        except:
